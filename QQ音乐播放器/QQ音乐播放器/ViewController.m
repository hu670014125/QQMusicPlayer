...
//    }
    ...
    CGFloat progress = (manager.currentTime - currentModel.beginTime) / (nextModel.beginTime - currentModel.beginTime);
    
    [self.lyricLabel setValue:@(progress) forKey:@"progress"];
    
    //设置歌词视图改变
    self.lyricView.currentLyricIndex = self.currentLyricIndex;
    
    //设置歌词视图变色
    self.lyricView.rowProgress = progress;
